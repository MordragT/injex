...
pub trait Injection: MemoryManipulation {
    fn pid(&self) -> i32;
    #[cfg(target_os = "linux")]
    fn inject<P: AsRef<Path>>(&self, library: P) -> InjectionResult<()> {
        ...
        let process = match LinuxProcess::new(self.pid()) {
            Ok(proc) => proc,
            Err(_) => return Err(InjectionError::ProcessNotFound(self.pid())),
        };
        ...
        match signal::kill(Pid::from_raw(self.pid()), Signal::SIGSTOP) {
            Ok(_) => (),
        ...
        let mut syscall_file = File::open(format!("/proc/{}/syscall", self.pid()))?;
        ...
        self.read(current_rip, &mut code_backup)?;
        let mut stack_backup = [0_u8; STACK_BACKUP_SIZE];
        self.read(current_rsp - STACK_BACKUP_SIZE, &mut stack_backup)?;
        ...
        self.write(current_rip, shell_code_buf.as_slice())?;

        match signal::kill(Pid::from_raw(self.pid()), Signal::SIGCONT) {
            Ok(_) => (),
            Err(e) => return Err(InjectionError::InternalError(e.to_string())),
        }
        Ok(())
